...
server <- function(input, output, session) {
  ...
  df_colors<-reactiveValues(df=dfcolors)

  #colors_solid<-reactiveValues(df=dfcolors[c(11:17),])
  #colors_gradient<-reactiveValues(df=dfcolors[c(1:10),])
  colors_solid_bordersom<-reactiveValues(df=dfcolors[c(11:17),])
  getsolid_col<-reactive({
    res<-lapply(newcolhabs$df, function(x) x(2))
    res1<-unlist(lapply(res, function(x) x[1]==x[2]))
    solid<-names(res1[res1==T])
    pic<-which(df_colors$df$val%in%solid)
    pic
  })
  getgrad_col<-reactive({
    ...
    pic<-which(df_colors$df$val%in%grad)
    pic
  })
  ...
  newcolhabs<- reactiveValues(df=list(
    "viridis" = get('viridis'),
    "plasma" = get('plasma'),
  ...
  observeEvent(input$testdata_rf,{
    vals$cur_testrf<-input$testdata_rf
  })
  ...
  observeEvent(input$data_rfY,{
    vals$cur_rfY<-input$data_rfY
  })

  observeEvent(input$rf_sup,{
    vals$cur_rfsup<-input$rf_sup
  })


  observeEvent(input$rf_type,{
    vals$cur_rftype<-input$rf_type
  })
  ...
  observeEvent(input$rf_test_partition,{
    vals$cur_partrf<-input$rf_test_partition
  })

  observeEvent(input$rf_models,{
    vals$cur_rf<-input$rf_models
  })


  observeEvent(input$som_test_pick,{vals$cur_partsom<-input$som_test_pick})
  ...
  observeEvent(input$choices_map,{
    vals$choices_map=input$choices_map
  })
  ...
  observeEvent(input$data_dtX,{
    vals$cur_data=input$data_dtX
  })
  ...
  output$srrgl_out<-renderRglwidget({
    ...
    srcol_layer=adjustcolor(getcolhabs(newcolhabs$df,input$srlayer_col,1), input$srlayer_lighten)
  ...
  output$ssrgl_out<-renderRglwidget({
    ...
    layer_shape=if(isTRUE(input$ssmap_layer)){
      attr(data,"layer_shape") } else{ NULL}
    sscol_layer=adjustcolor(getcolhabs(newcolhabs$df,input$sslayer_col,1), input$sslayer_lighten)
    sscol_base=adjustcolor(getcolhabs(newcolhabs$df,input$ssbase_col,1), input$ssbase_lighten)
           ...
           span(
             ...
             inline(
               pickerInput(
                 ...
                 selected=vals$choices_map, width="130px")
             ),
  ...
  output$map_get<-renderUI({
    ...
    inline(pickerInput("var_map",label = "select a variable",choices = choices,selected=vals$var_map, width="200px"))
  })
    ...
    column(12,
           fluidRow(
             ...
             span(
               ...
               div(
                 span("+ Color:",
                      inline(tags$div(class = "ident-picker", pickerInput(inputId = "srlayer_col",label = NULL,choices =   df_colors$df$val[getsolid_col()],choicesOpt = list(content =   df_colors$df$img[getsolid_col()]),options=list(container="body"),inline=F, width="40px", selected="gray")))
                 ),
                 span("+ Transp:",
    ...
    div("+ Coords style",
        column(12,' + Color:',
               pickerInput(inputId = "col_coords_stack",
                           label = NULL,
                           choices =   df_colors$df$val[getsolid_col()],
                           choicesOpt = list(content =   df_colors$df$img[getsolid_col()]), inline=T, width="75px")),
        column(12,
               '+ Size:',
      ...
      added_ss3d$df[[attr(x,"name")]]<-list(
        div(
          ...
          column(12,span("+ Palette:",
                         pickerInput(inputId = paste0("pt_palette",i),label = NULL,choices = df_colors$df$val,choicesOpt = list(content = df_colors$df$img),options=list(container="body"),inline=T, width="55px")
          )),
          column(12,span("+ Size:",
    ...
    column(12,
           fluidRow(
             ...
             span(
               ...
               div(
                 span("+ Color:",
                      inline(tags$div(class = "ident-picker", pickerInput(inputId = "ssbase_col",label = NULL,choices =   df_colors$df$val[getsolid_col()],choicesOpt = list(content =   df_colors$df$img[getsolid_col()]),options=list(container="body"),inline=F, width="40px", selected="white")))
                 ),
                 span("+ Transp:",
        ...
        span(
          ...
          div(
            span("+ Color:",
                 inline(tags$div(
                   class = "ident-picker",
                   pickerInput(inputId = paste0("ssextra_layer_col",i),label = NULL,choices =   df_colors$df$val,choicesOpt = list(content =   df_colors$df$img),options=list(container="body"),inline=F, width="40px", selected="gray")
                 ))
            ),
            span("+ Transp:",
    ...
    column(12,
           fluidRow(
             ...
             span(
               ...
               div(
                 span("+ Color:",
                      inline(tags$div(class = "ident-picker", pickerInput(inputId = "sslayer_col",label = NULL,choices =   df_colors$df$val[getsolid_col()],choicesOpt = list(content =   df_colors$df$img[getsolid_col()]),options=list(container="body"),inline=F, width="40px", selected="gray")))
                 ),
                 span("+ Transp:",
  ...
  output$cmap<-renderUI({
    ...
    fluidRow(
      radioGroupButtons(
        ...
        ),choiceValues =list("discrete","interpolation","raster"), status="button_active",justified =T, selected=vals$cmap)
    )
  })
  ...
  output$map_00_palette<-renderUI({
    ...
    column(12,
           ...
           padding-bottom:1px;
          ",
          "+ Data palette:",pickerInput(inputId = "pt_palette",label = NULL,choices = df_colors$df$val,choicesOpt = list(content = df_colors$df$img),options=list(container="body"),inline=T, width="75px"))
    )
  })
  ...
  output$show_map_axes<-renderUI({
    ...
    div(
      ...
      column(12,' +	Legend:',
             column(12,
                    '+ label',
                    inline(textInput("map_legend",NULL, "Value", placeholder = "legend", width="75px"))
             ))
  ...
  output$map_side04<-renderUI({
    req(!isTRUE(input$surface_map))
    req(!isTRUE(input$mantel_map))
    div(class="well2",
        span("+",  tipify(bsButton('shp_view', span(icon("fas fa-layer-group"),"Create base/layer shape"),style  = "shp_button"),"Shape file tools. Use this tool to create Base and Layer shapes from Shapefiles. The mandatory file extensions needed for a shapefile are .shp, .shx and .dbf. But the optional files are: .prj, .xml, .sbn and .sbx"
        )
        )
    )
...
tipify(span('Breakpoints'), "The break points computed", placement = "right")
             ...
             span(
               ...
               inline(
                 conditionalPanel("input.cmap=='discrete'",{
                   span(
                     span("+ Color:",
                          inline(tags$div(class = "ident-picker",pickerInput(inputId = "base_col",label = NULL,choices =   df_colors$df$val[getsolid_col()],choicesOpt = list(content =   df_colors$df$img[getsolid_col()]),options=list(container="body"),inline=F, width="40px", selected="white")))
                     ),
                     span("+ Transp:",
  ...
  output$map_1b_discrete<-renderUI({
    ...
    column(12,
           div(class="well2",
               ...
               column(12,
                      '+ Size:',
                      inline(numericInput("pt_points",NULL, 1, width="75px"))
               ),
    ...
    column(12,
           div(
             ...
             span(
               ...
               div(
                 span("+ Color:",
                      inline(tags$div(class = "ident-picker", pickerInput(inputId = "layer_col",label = NULL,choices =   df_colors$df$val[getsolid_col()],choicesOpt = list(content =   df_colors$df$img[getsolid_col()]),options=list(container="body"),inline=F, width="40px", selected="gray")))
                 ),
                 span("+ Transp:",
  ...
  output$scatter_out<-renderUI({
    ...
    z00<-z0<-if(isTRUE(input$scatter_4D)){
      colvar} else{z}
    col= getcolhabs(newcolhabs$df,input$pt_palette,n=100)
    ...
    col_layer=adjustcolor(getcolhabs(newcolhabs$df,input$layer_col,1), input$layer_lighten)
    col_base=adjustcolor(getcolhabs(newcolhabs$df,input$base_col,1), input$base_lighten)
    div(
      renderPlot({
        ...
        get_scatter3D(z,
                      ...
                      pt_col=pt_col,
                      fac_col=if(length(input$col_factor)>0){   getcolhabs(newcolhabs$df,input$col_factor,1) } else{ NULL},
                      zlab=input$var_map,
  ...
  observeEvent(input$tour_databank,{guide1$highlight('MENU_databank', session = NULL, is_id = TRUE)$init()$start()})
  ...
  observeEvent(input$guide1_cicerone_next$highlighted,{
    ...
    req(input$guide1_cicerone_next$highlighted=='MENU_databank')
      updateTabItems(session, "tabs", "menu_upload")
    delay(500,{   guide1$highlight("MENU_databank")$init()$start(step = 3)})
...
observeEvent(input$help_databank,{
     ...
     #uiOutput("tour_cur"),
      ...
      div(class = "col-sm-6", style = "padding-left:0",
        p(style="text-align:left; color:black",
          inline(span(inline(gettitle())))),
      ),
      div(class = "col-sm-6", style = "padding-right:0",
        p(style="text-align:right; color:red",
          actionButton("savepoints_button",strong("Savepoints"),style="font-size: 10px; background: SeaGreen; color: white", icon=icon("fas fa-download"))
        )
      )
  ...
  getdata_map <- reactive({
    ...
    factors<-attr(data, "factors")
    coords<-attr(data,"coords")
    vals$saved_labels<-factors
    vals$saved_layer_shape<-attr(data,"layer_shape")
    vals$saved_base_shape<-attr(data,"base_shape")
    vals$saved_coords<-coords
    data
  })
  ...
  getbox <- reactive({
    ...
    #rownames(res)<-rownames(vals$saved_labels)[pic]
  ...
  observeEvent(input$bmu_bgpalette,{bmu_bgpalette$df<-input$bmu_bgpalette})
  observeEvent(input$bmu_dotlabel,{bmu_dotlabel$df<-input$bmu_dotlabel})
  observeEvent(input$bmu_facpalette,{bmu_facpalette$df<-input$bmu_facpalette})
  observeEvent(input$bmu_symbol,{bmu_symbol$df<-input$bmu_symbol})
  observeEvent(input$bmu_symbol_size,{vals$bmu_symbol_size<-input$bmu_symbol_size})
  observeEvent(input$bmu_cexvar,{vals$bmu_cexvar<-input$bmu_cexvar})
  observeEvent(input$bmu_bg_transp,{vals$bmu_bg_transp<-input$bmu_bg_transp})
  observeEvent(input$bmu_border_grid,{bmu_border_grid$df<-input$bmu_border_grid})
  observeEvent(input$bmu_factors,{vals$bmu_factors<-input$bmu_factors})
  ...
  observeEvent(input$bmu_leg_transp,{vals$bmu_leg_transp<-input$bmu_leg_transp})
  ...
  gettitle<-reactive({
    div(style="color: #05668D", id="menu-title",
        ...
        switch(input$tabs,
               ...
               'menu_explore'={
                 h4(strong("Pre-processing & Descriptive tools", pophelp(NULL,"Explore and modify data along boxplots, histograms and classic ordination techniques")))
               },
               'menu_som'={
  ...
  output$panel_main<-renderUI({
    ...
    data<-vals$saved_data[[input$data_upload]]
    column(12,style=paste(border_alert,";background: white"),
           fluidRow(
             navbarPage("",id='desc_options',
                        tabPanel('Summaries',
                          ...
                          inline(
                            tipify(
                              pickerInput(
                                inputId = "rda_spcolor",
                                label = NULL,
                                selected=  df_colors$df$val[getsolid_col()][4],
                                choices =   df_colors$df$val[getsolid_col()],
                                choicesOpt = list(
                                  content =   df_colors$df$img[getsolid_col()]
                                ),
                                width="75px",
                                options=list(container="body")
                              ), "Variable Color.")))
  ...
  output$mds_options<-renderUI({
    ...
    div(
        ...
        column(12,
               ...
               div(
                 span("+ Color:",
                      inline(
                        tipify(
                          pickerInput(inputId = "mds_colpalette",label = NULL,choices = df_colors$df$val,choicesOpt = list(content = df_colors$df$img),options=list(container="body"),selected=df_colors$df$val[1], width='75px'), "Symbol palette. Choose a gradient to color observations by a factor")))),
      ...
      conditionalPanel("input.mds_show_labels % 2",{
        column(12,
               div(span("+ Factor:",
                        inline(tipify(pickerInput("mds_labfactor",NULL,choices = colnames(attr(getdata_upload(),"factors")), width="125px"),  "label classification factor")
                        ))),
               div(span("+ Lab Color:",
                        inline(tipify(
                          pickerInput(
                            ...
                            label = NULL,
                            selected=  df_colors$df$val[12],choices =   df_colors$df$val,choicesOpt = list(content =   df_colors$df$img),width="75px",
                            options=list(container="body")
  ...
  output$mds_fac_palette<-renderUI({
    col<-getcolhabs(newcolhabs$df,input$mds_colpalette,2)
    req(col[1]!=col[2])
    div(
      span("+ Factor:",
           inline(tipify(pickerInput("mds_symbol_factor",NULL,choices = rev(colnames(attr(getdata_upload(),"factors"))), width='125px'), "symbol classification factor"))))
  })
  ...
  output$pca_options<-renderUI({
    div(
        ...
        column(12,
               ...
               div(
                 span("+ Color:",
                      inline(
                        tipify(
                          pickerInput(inputId = "pca_colpalette",label = NULL,choices = df_colors$df$val,choicesOpt = list(content = df_colors$df$img),options=list(container="body"),selected=df_colors$df$val[1], width='75px'), "Symbol palette. Choose a gradient to color observations by a factor")))),
      ...
      conditionalPanel("input.pca_show_labels % 2",{
        column(12,
               div(span("+ Factor:",
                        inline(tipify(pickerInput("pca_labfactor",NULL,choices = colnames(attr(getdata_upload(),"factors")), width="125px"),  "label classification factor")
                        ))),
               div(span("+ Lab Color:",
                        inline(tipify(
                          pickerInput(
  ...
  output$pca_fac_palette<-renderUI({
    col<-getcolhabs(newcolhabs$df,input$pca_colpalette,2)
    req(col[1]!=col[2])
    div(
      span("+ Factor:",
           inline(tipify(pickerInput("pca_symbol_factor",NULL,choices = rev(colnames(attr(getdata_upload(),"factors"))), width='125px'), "symbol classification factor"))))
  })
  ...
  output$rda_options<-renderUI({
    ...
    div(
        ...
        column(12,
               ...
               div(
                 span("+ Color:",
                      inline(
                        tipify(
                          pickerInput(inputId = "rda_colpalette",label = NULL,choices = df_colors$df$val,choicesOpt = list(content = df_colors$df$img),options=list(container="body"),selected=df_colors$df$val[1], width='75px'), "Symbol palette. Choose a gradient to color observations by a factor")))),
      ...
      conditionalPanel("input.rda_show_labels % 2",{
        column(12,
               div(span("+ Factor:",
                        inline(tipify(pickerInput("rda_labfactor",NULL,choices = colnames(attr(getdata_upload(),"factors")), width="125px"),  "label classification factor")
                        ))),
               div(span("+ Lab Color:",
                        inline(tipify(
                          pickerInput(
  ...
  output$rda_fac_palette<-renderUI({
    col<-getcolhabs(newcolhabs$df,input$rda_colpalette,2)
    req(col[1]!=col[2])
    div(
      span("+ Factor:",
           inline(tipify(pickerInput("rda_symbol_factor",NULL,choices = rev(colnames(attr(getdata_upload(),"factors"))), width='125px'), "symbol classification factor"))))
  })
  ...
  output$upload_tools<-renderUI({
    column(12,
           splitLayout(cellWidths = c("35%","65%"),
                       column(12,uiOutput("upload_selection")
                       ),
  ...
  output$upload_selection<-renderUI({
    ...
    div(
      ...
      pickerInput("data_upload",NULL,choices=names(vals$saved_data),  selected=vals$cur_data, options=list(container="body","style-base" = "form-control", style = "")))
  ...
  output$tools_upload<-renderUI({
    ...
    div(
      id="upload_tools",
      dropdownButton(icon = icon("fas fa-list"), status ="button_active" ,width="400px",
                     column(12,style="overflow-x: scroll;height:450px;overflow-y: scroll; margin-left: -10px",
                            column(12,
                                   ...
                                   conditionalPanel("input.match % 2",{
                                     column(12,style="background:white",
                                            ...
                                            conditionalPanel("input.filterby=='a factor level'",{
                                              column(12,splitLayout(selectInput("filter_data", span("Factor",tipify(a(icon("fas fa-question-circle")),"The factor to be used as filter", options = list(container="body"))),c("none", colnames(attr(vals$saved_data[[input$data_upload]],"factors"))), selectize=T),conditionalPanel("input.filter_data != 'none'",{uiOutput("cutlevel_obs")})))}),
                                            conditionalPanel("input.filterby=='IDs from another Datalist'",{
                                              column(12,selectInput("filter_datalist", span("Datalist::",tipify(a(icon("fas fa-question-circle")),"The Datalist from which the ID will be used as filter", options = list(container="body"))),choices = c("none",names(vals$saved_data)), selectize=T))}),
                                            conditionalPanel("input.filterby=='individual selection'",{
                            ...
                            column(12,strong("Removed observations:"),style='white-space: normal;',
                                   ...
                                   renderPrint({
                                     ...
                                     IDs1[which(!IDs1%in%IDs2)]}))), inline = T, circle=F,tooltip = tooltipOptions(title = "Select observations", placement="bottom")),
      tags$style(".nav {z-index:999}"),

      tags$style("#upload_tools .dropdown-menu {border: 2px solid SeaGreen;margin-left: -50px; margin-top: 10px; background: #e7f2ecfc;z-index:9999}"),
      dropdownButton(icon = icon("fas fa-list fa-rotate-90",verify_fa = FALSE), status ="button_active" , inputId = "dropID_selecvar",width="350px",
                     inline = T, circle=F,tooltip = tooltipOptions(title = "Select variables", placement="bottom"), uiOutput("selectVar")),
      dropdownButton(icon = icon("fas fa-hammer"), status ="button_active" ,  inline = T, circle=F,tooltip = tooltipOptions(title = "Transformation toobox", placement="bottom"),width="350px",
                     uiOutput("pop_transform")
      ),
      span(id="drop_na",dropdownButton(label=img(src=na_icon,height='15',width='15'), status ="button_active" ,  inline = T, circle=F,tooltip = tooltipOptions(title = "Data imputation", placement="bottom"),width="350px",
                                       inputId="na_pop",
                                       uiOutput("pop_na")
      )),
      tags$style("#drop_na .dropdown-menu {border: 2px solid SeaGreen;margin-left: -80px; margin-top: 10px; background: #e7f2ecfc}"),

      span(id="drop_split",dropdownButton(label=img(src=split_icon,height='20',width='20'), status ="button_active",inline=T, circle=F,tooltip=tooltipOptions(title="Split datalist in training and Test data", placement="bottom"),width="300px",inputId='dropID_split',
                                          uiOutput("pop_split")
      )),
      tags$style("#drop_split .dropdown-menu {border: 2px solid SeaGreen;margin-left: -100px; margin-top: 10px; background: #e7f2ecfc}"),
      span(id="drop_tag",dropdownButton(
        icon = icon("fas fa-tags"), status ="button_active",inline=T, circle=F,tooltip=tooltipOptions(title="Edit factor levels", placement="bottom"),width="400px",inputId="dropID_tag",
        uiOutput("pop_tag")

      )),
      tags$style("#drop_tag .dropdown-menu {border: 2px solid SeaGreen;margin-left: -260px; margin-top: 10px; background: #e7f2ecfc}"),
      span(id="drop_palette",inline(dropdown(

        icon=icon("fas fa-palette"),
        status ="button_active",tooltip=tooltipOptions(title='Create Color palette', placement="bottom"),width="400px",inputId="dropID_palette",
        uiOutput("pop_palette")

      ))),
      tags$style("#sw-content-dropID_palette {border: 2px solid SeaGreen;margin-left: -260px;  background: #e7f2ecfc;z-index:9999}"),

      inline(
        uiOutput("bttn_save_changes", inline=T)
      ),
      inline(
        uiOutput("info_save_changes", inline=T)
      )
    )
  })
  created_pal<-reactiveValues(df=NULL)
  output$pop_palette<-renderUI({
    column(12,
           h4(strong("Create color palette"),style="border-bottom: 1px solid SeaGreen;"),
           div(
             span(
               inline(uiOutput("palette_new")),
               #inline(uiOutput("newpalette_created"))
               inline(uiOutput("custom_palette"))
             )
           ),
           div(uiOutput("palette_creation")),

    )
  })

  output$custom_palette<-renderUI({
    pic<-which(!df_colors$df$val%in%c(
      "matlab.like2",'viridis', 'plasma',"Rushmore1","FantasticFox1",'Grays',"heat",'Purples',"Blues",'Greens',"black","gray","royalblue", "firebrick","forestGreen",'goldenrod3',"white"
    ))
    req(length(pic)>0)
    div(
      inline(pickerInput(inputId = "available_palette",
                         label = NULL,
                         choices = df_colors$df$val[pic],
                         choicesOpt = list(content = df_colors$df$img[pic]),  width="100px")),
      tipify( actionButton("delete_palette",icon("far fa-trash-alt")),"Click to delete the selected palette")

    )})
  observeEvent(input$delete_palette,{
    pic<-which( names(newcolhabs$df)==input$available_palette)
    newcolhabs$df[[pic]]<-NULL
    pic<-which(df_colors$df$val==input$available_palette)
    df_colors$df<-df_colors$df[-pic,]
  ...
  output$palette_creation<-renderUI({
    ...
    div(
      fluidRow(
        inline(div(style="width: 40px",
                   tipify(colourpicker::colourInput("col", NULL, "purple",showColour='background'),"Click to change color"),
        )),tipify(actionButton("add_color",icon("fas fa-plus")),"Click to include the color")
      ),
      div(
  ...
  output$palette_new<-renderUI({
    req(!is.null(created_pal$df))
    tipify(span(
      actionButton("palette_new",span(icon("fas fa-plus-square"),icon("fas fa-paint-brush")))

    ),'Create new palette')
  })
  observeEvent(input$palette_new,{
    created_pal$df<-NULL
  })
  ...
  observeEvent(input$add_palette,{
    palette_name<-paste("palette",length(newcolhabs$df))
    ...
    df_colors$df<-rbind(df_colors$df,data.frame(val=palette_name,img= sprintf(paste0(img(src = palette1, height = '20',width = '70',style="margin-top:-40px;margin-bottom:-40px;margin-left:-20px;margin-right:-20px;") ))))
    newcolhabs$df[[palette_name]]<-colorRampPalette(newpalette$df_colors)
    created_pal$df<-T
  ...
  output$newpalette_img<-renderUI({
    validate(need(length(newpalette$df_colors)>0,"No color added to palette"))


    res<-lapply(1:maxcol(),function(i)
      output[[paste0("col",i)]]<-renderUI({
        actionButton(paste0("col",i), NULL, style= paste('height: 20px;background:',newpalette$df_colors[i]))
      })
    )
    ...
    div(
      div(strong("Pre-saved colors:")),
      div(
        fluidRow(
          div(lapply(
            1:12, function (i) inline(uiOutput(paste0("col",i), inline=T))
          )),
          div(lapply(
            13:24, function (i) inline(uiOutput(paste0("col",i), inline=T))
          )),
          div(lapply(
            25:36, function (i) inline(uiOutput(paste0("col",i), inline=T))
          )),
          div(lapply(
            37:48, function (i) inline(uiOutput(paste0("col",i), inline=T))
          )),

          span(
            tipify(actionButton("add_palette",span(icon("fas fa-palette"),icon("fas fa-plus"))),"Click to create a palette with the colors above. The new palette will be available in all color inputs"),
            tipify(actionButton("restart_palette",span(icon("fas fa-undo"))),"Click to restart")
          ),
          div(uiOutput("maxcolor"))
        )
      ),
    )
  })
  ...
  output$pop_tag<-renderUI({
    ...
    column(12,
           h4(strong("Edit factors"),style="border-bottom: 1px solid SeaGreen;"),
           div(
             splitLayout(
  ...
  output$pop_na<-renderUI({
    ...
    column(12,
           h4(strong("Data imputation",tipify(icon("fas fa-question-circle"), "Impute missing values")),style="border-bottom: 1px solid SeaGreen;"),
  ...
  output$pop_split<-renderUI({
    column(12,
           h4(strong("Split tools"),style="border-bottom: 1px solid SeaGreen;"),
           column(12,
           ...
           fluidRow(style="background: white",column(12,tipify(actionButton("addpart",label=icon("fas fa-plus")),"Add partition  as a factor in Factor-Attribute. You can later choose the factor which you want to use to constrain training and testing data. ", options=list(container="body")))
           ),
           column(12,
                  splitLayout(
                    column(12,
  ...
  output$bttn_save_changes<- renderUI({
    inline(
      tipify(actionButton("tools_save", icon("fas fa-save"),style  = if(sum(vals$bagdata==c(T,T))==2){"button_active"} else{"animation: glowing2 1000ms infinite;"}),"Transformed/ filtered data can be saved as a new datalist or overwrite the original datalist",options=list(container="body"))
    )
  ...
  output$selectVar<-renderUI({
    ...
    column(12,style="overflow-x: scroll;height:350px;overflow-y: scroll",
           ...
           h4(strong("Select/Remove variables"),style="border-bottom: 1px solid SeaGreen;"),
           uiOutput("remove_sp"),
  ...
  output$bank_tools<-renderUI({
    column(12,
           fluidRow(style="height: 85px; margin-top: 0px; margin-bottom: -20px",splitLayout(
             splitLayout(
  ...
  output$bank_input<-renderUI({
    if(length(vals$saved_data)<1) {
      ...
      div(
        ...
        selectInput("data_bank",NULL,choices=names(vals$saved_data), selectize=T, selected=vals$cur_data)
      )
    }
  })
  output$tools_bank<-renderUI({
    req(length(vals$saved_data)>0)
    uiOutput('stats_attributes')
  })
  output$stats_attributes<-renderUI({
    ...
    splitLayout(cellWidths = c("90%","10%"),
                       ...
                       if(is.null(attr(data,"som"))){
                         radioGroupButtons("view_datalist",NULL,
                                           choiceNames =list(
                                             ...
                                             div(id="004","B",icon("fas fa-map")),
                                             div(id="005","L",icon("far fa-map"))),
                                           choiceValues =list("data","factors","coords","base_shape","layer_shape"),status="button_active",selected = vals$curview_databank
                         )
                       } else{radioGroupButtons(
                         "view_datalist",NULL,
                         choiceNames =list(
                           div(id="001",icon("fas fa-archive")),
                           div(id="c002",icon("fas fa-boxes")),
                           ...
                           div(id="004","B",icon("fas fa-map")),
                           div(id="005","L",icon("far fa-map")),
                           div(id="006",icon("fas fa-braille"))),
                         choiceValues =list(
                           "data","factors","coords","base_shape","layer_shape","som"
                         ),status="button_active",selected=vals$curview_databank)}
                ),
                ...
                bsPopover("004",NULL,"Base-shape-Attribute"),
                bsPopover("005",NULL,"Layer-shape-Attribute"),
             ...
             div(
               ...
               inline(
                 dropdownButton(icon=icon("fas fa-share-alt"),
                                ...
                                inputId ="edit_datalist",
                                tooltip = tooltipOptions(title = "Merge the selected Datalist with another one by ID", placement="bottom"),
                                div(
                                  splitLayout(
                                    cellWidths = c("50%","10%"),
                                    selectInput("to_merge", "select a Datalist", choices=names(vals$saved_data)[names(vals$saved_data)!=input$data_bank]),
                                    column(12,style="margin-top: 20px",
                                           tipify(actionButton("merge_data",icon("fas fa-plus")),"Click to the merge the selected Datalist to the current one")
                                    )
                                  )
                                )
                 )
    ...
    fluidRow(style="overflow-x: scroll;",
             div(class="datalist_tools",h5(
               ...
               inline(
                 div(
                   inline(
                     tipify(actionButton("downcenter_factors",icon("fas fa-download")),"Download table")
                   ),
                   dropdownButton(icon=icon("fas fa-edit"),
                                  status ="button_active" ,  inline = T, circle=F,tooltip = tooltipOptions(title = "Remove a factor", placement="bottom"),
                                  div(
                                    splitLayout(cellWidths = c("30%","10%"),
                                                selectInput("remove_factor", "select a factor", choices=colnames(attr(vals$saved_data[[input$data_bank]],"factors"))),
                                                column(12,style="margin-top: 20px",
                                                       tipify(actionButton("delete_factor",icon("fas fa-trash-alt")),"Click to remove")
                                                )
                   ...
                   dropdownButton(icon=icon("fas fa-share-alt"),
                                  status ="button_active" ,  inline = T, circle=F,
                                  tooltip = tooltipOptions(title = "Add a factor from another Datalist", placement="bottom"),
                                  div(
                                    splitLayout(
                                      cellWidths = c("30%",'30%',"10%"),
                                      selectInput("import_datalist", "select a factor", choices=names(vals$saved_data)[names(vals$saved_data)!=input$data_bank]),
  ...
  output$import_factor_yes<-renderUI({
    ...
    column(12,
           ...
           p(strong("Source:"),em(input$import_datalist,"::",input$import_factor,style="color: SeaGreen")),
  ...
  observeEvent(input$add_factor_yes,{
    factor_i<-attr(vals$saved_data[[input$import_datalist]],"factors")
    fac<-factor_i[,input$import_factor]
    factor_t<-attr(vals$saved_data[[input$data_bank]],"factors")
  ...
  output$import_factor<-renderUI({
    ...
    selectInput("import_factor", "select a factor", choices=colnames(attr(vals$saved_data[[input$import_datalist]],"factors")))
  ...
  output$viewbase<-renderUI({
    column(12,
           ...
           h5(strong("Base-Shape-Attribute"),actionButton("delete_base_shape",icon("fas fa-trash-alt"))),
  ...
  output$viewlayer<-renderUI({
    if(is.null(attr(getdata_bank(),"layer_shape"))) {fluidRow(
      ...
      fluidRow(
        h5(strong("Layer-Attribute",actionButton("delete_layer_shape",icon("fas fa-trash-alt")))),
        renderPlot({
    ...
    showModal(
      modalDialog(
        ...
        column(12, style="margin-top: 20px; margin-bottom: 20px",
               h5(strong("Remove factor:",style="color: red"),em(input$remove_factor,style="color: gray")),
               bsButton("delete_factor_yes",icon=icon("far fa-trash-alt"),"Confirm", style="button_active",block=T))
        ,
        easyClose = T,
        size = "s"
  ...
  observeEvent(input$delete_factor_yes,{
    ...
    removeModal()
    updateRadioGroupButtons(session,"view_datalist", selected="factors")
  ...
  observeEvent(input$delete_base_shape_yes,{
    ...
    removeModal()
    updateRadioGroupButtons(session,"view_datalist", selected="base_shape")
  })
  ...
  observeEvent(input$delete_layer_shape_yes,{
    ...
    removeModal()
    updateRadioGroupButtons(session,"view_datalist", selected="layer_shape")
  })
      ...
      conditionalPanel("input.hand_facs=='Plot'",{
        fluidRow(
          column(12,
                 splitLayout(cellWidths = c('25%','25%',"50%"),
                             ...
                             column(12,
                                    ...
                                    pickerInput(inputId = "fac_palette",
                                                label = NULL,
                                                choices =     df_colors$df$val[getgrad_col()],
                                                choicesOpt = list(content =     df_colors$df$img[getgrad_col()]), options=list(container="body")))
                             ,
                 ...
                 renderPlot(pfactors(
                   attr(data, 'data.factors')[input$sel_ommitedfac], palette=input$fac_palette, newcolhabs=newcolhabs$df
                 ))
          )
        )
      }),
  ...
  bag_extralayer<-reactive({
    ...
    name0<-'Extra-Layer'
    name1<-paste0(name0," (",bag,")")
    if(name1%in%names(attr(vals$saved_data[[input$data_map]],"extra_shape")))
    {
      repeat{
        bag<-bag+1
        name1<-paste0(name0," (",bag,")")
        if(!name1%in%names(attr(vals$saved_data[[input$data_map]],"extra_shape"))) break
      }
    }
    paste0(name0," (",bag,")")
  ...
  output$data_create<-renderUI({
    ...
    res<-switch (vals$hand_save,
                 ...
                 "Save changes"= bag_name(),
                 "Include binary columns in the Data-Attribute"=  bag_name(),
                 "Include ordinal variables in the Data-Attribute"=  bag_name(),
  ...
  output$data_over<-renderUI({
    ...
    res<-switch (vals$hand_save,
                 ...
                 'Create data list from aggregation results' = action,
                 'case' =  selectInput("agg_over", NULL,choices=c(names(vals$saved_data)),selectize=T),
  ...
  mergedata<-reactive({
    data1<-getdata_bank()
    data2<-vals$saved_data[[input$to_merge]]
    newdata<-cbind(data1,data2[rownames(data1),, drop=F])
  ...
  saverf<-reactive({
    ...
    if(input$hand_save=="create"){
      temp<-list(temp)
      names(temp)<-input$rf_newname
      attr(vals$saved_data[[input$data_rfX]],"rf")[input$rf_newname]<-c(temp,attr(vals$saved_data[[input$data_rfX]],"rf")[[input$rf_newname]])
      cur<-input$rf_newname
    } else{
      temp<-list(temp)
      names(temp)<-input$rf_over
      attr(vals$saved_data[[input$data_rfX]],"rf")[input$rf_over]<-temp
      cur<-input$rf_over
    }
    updateTabsetPanel(session, "rf_tab", vals$currftab)
    updateSelectInput(session,"rf_models", selected=cur,choices=c(names(attr(vals$saved_data[[input$data_rfX]],"rf"))))
    vals$cur_rf<-cur

  })
  save_dataobs<-reactive({
    data=getdata_upload()
    ...
    if(vals$hand_save=="Create a datalist from the selected observations") {
      vals$saved_data[[input$selobs_newname]]<-data
      vals$cur_data<-input$selobs_newname
    } else{
      vals$saved_data[[input$selobs_over]]<-data
      vals$cur_data<-input$selobs_over
    }
  })
  datalistrf<-reactive({

    temp<-if(input$rfpred_which=="Datalist"){
      vals$saved_data[[input$predrf_new]][,vals$rf_sigs$variable]
    } else{getdata_rfX()[,vals$rf_sigs$variable]}

    datao<-if(input$rfpred_which=="Datalist"){
      vals$saved_data[[input$predrf_new]]
    } else{getdata_rfX()}
    if(input$hand_save=="create") {
      temp<-data_migrate(datao,temp,input$rf_newname)
      vals$saved_data[[input$rf_newname]]<-temp
      vals$cur_data<-input$rf_newname
    } else{
      temp<-data_migrate(datao,temp,input$rf_over)
      vals$saved_data[[input$rf_over]]<-temp
      vals$cur_data<-input$rf_over
    }
  })
  datalistrf_predicions<-reactive({
    ...
    if(input$hand_save=="create") {
      temp<-data_migrate(datao,temp,input$rf_predictions_newname)
      vals$saved_data[[input$rf_predictions_newname]]<-temp
    } else{
      temp<-data_migrate(datao,temp,input$rf_predictions_over)
      vals$saved_data[[input$rf_predictions_over]]<-temp
    }

  })
  datalistrf_prederrors<-reactive({
    temp<-switch(vals$RF_results$modelType,
                 "Regression"=data.frame(RFerror_reg(predall_rf(),RF_observed())),
                 "Classification" =data.frame(RFerror_class(predall_rf(),RF_observed())))
    ...
    if(input$hand_save=="create") {
      temp<-data_migrate(datao,temp,input$rf_pred_errors_newname)
      vals$saved_data[[input$rf_pred_errors_newname]]<-temp
    } else{
      temp<-data_migrate(datao,temp,input$rf_pred_errors_over)
      vals$saved_data[[input$rf_pred_errors_over]]<-temp
    }
  ...
  datalistrferrors<-reactive({
    ...
    temp<-data_migrate(getdata_rfX(),temp,"newdata_rf")
    if(input$hand_save=="create") {
      vals$saved_data[[input$rferrors_newname]]<-temp
    } else{
      vals$saved_data[[input$rferrors_over]]<-temp
    }
  })

  datalistrfinter<-reactive({
    a<-rf_interactions_frame$df[1:input$rfinter_k2,"variable"]
    b<-as.character(rf_interactions_frame$df[1:input$rfinter_k2,"root_variable"])
    pic<-unique(c(a,b))

    temp<-getdata_rfX()[, pic]
    temp<-data_migrate(getdata_rfX(),temp,"newdata_rf")
    if(input$hand_save=="create") {
      vals$saved_data[[input$rfinter_newname]]<-temp
      vals$cur_data<-input$rfinter_newname
    } else{
      vals$saved_data[[input$rfinter_over]]<-temp
      vals$cur_data<-input$rfinter_over
    }
  })
  savediv<-reactive({
    divInds<-divI()
  ...
  bag_name<-reactive({
    ...
    res<-textInput("data_newname", NULL,paste(gsub(".csv","",attr(getdata_upload(), "filename")) ,bag))
    res
  })

  bag_RFname<-reactive({
    bag<-1
    name0<-paste0("RF (",input$data_rfY,"::",input$rf_sup,'~',input$data_rfX,")")
  ...
  output$preview<-renderUI({
    if(vals$hand_save=="Save new som in"){
      bmu<-data.frame(vals$som_results$unit.classif)
      rownames(bmu)<-rownames(vals$som_results$data[[1]])
      colnames(bmu)<-paste0('bmu_',input$model_newname)
      column(12,
             style="overflow-x: scroll;height:300px;overflow-y: scroll",
             renderPrint(cbind(attr(getdata_som(),"factors")[rownames(bmu),,drop=F],bmu))
      )

    } else if(vals$hand_save=="Save changes")
    {
      column(12,
             style="overflow-x: scroll;height:300px;overflow-y: scroll",
             column(12,
                    renderTable(
                      getdata_upload(), bordered = T,rownames =T,striped=T, spacing ="xs"
                    )))
    } else if(vals$hand_save=="Save Clusters")
    {
      hc <- phc()
      ...
      column(12,
             style="overflow-x: scroll;height:300px;overflow-y: scroll",
             renderPrint(cbind(attr(getdata_hc(),"factors")[rownames(temp),,drop=F],temp)))
    }    else if(vals$hand_save=="Create data list from aggregation results")
    {
      column(12,
             style="overflow-x: scroll;height:300px;overflow-y: scroll",
             column(12,
                    renderTable(
                      aggreg(), bordered = T,rownames =T,striped=T, spacing ="xs"
                    )
             ))
    } else if(vals$hand_save=="Create a datalist with the variables selected in the Random Forest Explainer")
    {
      column(12,
             style="overflow-x: scroll;height:300px;overflow-y: scroll",
             column(12,
                    renderTable(
                      getdata_rfX()[,vals$rf_sigs$variable],striped=T, spacing ="xs"
                    )

             ))

    } else if(vals$hand_save=="Include binary columns in the Data-Attribute"|vals$hand_save=="Include ordinal variables in the Data-Attribute"){
      data<-vals$saved_data[[input$data_upload]]

      column(12,
             style="overflow-x: scroll;height:300px;overflow-y: scroll",
             column(12,renderTable(
               getclassmat(attr(data, 'data.factors')[,input$to_classmat,drop=F]),striped=T, spacing ="xs"
             )))
    }
    ...
    downloadHandler(
      ...
      content = function(file) {
        withProgress(message = "Preparing the download ...",
                     ...
                     max = 1,
                     {
                       myBookmarks=isolate(reactiveValuesToList(vals))
  ...
  output$filter_box1<-renderUI({
    selectInput('filter_box1',"Filter:",choices = c("none", colnames(attr(getdata_upload(),"factors"))),selected=filter_box1_cur$df)
  })
  ...
  output$box_factor<-renderUI({
    selectInput('box_factor',"Factor:",choices = rev(colnames(attr(getdata_upload(),"factors"))),selected=box_factor_cur$df)
  })
    ...
    fluidRow(class="map_control_style",style="color: #05668D",
             ...
             div(span("+ Palette:",inline(
               pickerInput(inputId = "box_palette",
                           ...
                           choicesOpt = list(content = df_colors$df$img), options=list(container="body"), width="75px")
             ))),
             div(span("+ Margins:",
             ...
             conditionalPanel("input.show_outs % 2",{
               column(12,span("+",inline(
                 ...
                 conditionalPanel("input.showout % 2",{
                   div(
                     pickerInput("out_label",NULL,choices=colnames(attr(getdata_upload(),"factors")), width="95px")
                   )
                 })
               )))
             }),
  ...
  rf_results<-reactiveValues(df=F)

  observe({
    req(isFALSE(rf_results$df))
    req(input$data_rfX)
    req(length(attr(vals$saved_data[[input$data_rfX]],"rf"))>0)
    rf_results$df<-T
    insertTab("rf_tab", target="rf_tab1", position="after",
              tabPanel(
    ...
    insertTab("rf_tab", target="rf_tab2", position="after",
              tabPanel(strong("3. Predict"),value="rf_tab3",style="background: white",
                       uiOutput("RF_predictions")
              )
    )
  ...
  output$RF_results<-renderUI({
    ...
    tabsetPanel(
      ...
      tabPanel(strong("2.5 Confusion Matrix"),
               column(12,style="background: white", uiOutput("CMres")))
           ...
           splitLayout(cellWidths = c("30%",'10%'),
                       column(12,
                              ...
                              popify(pickerInput(inputId = "rfpalette",
                                                 label = NULL,
                                                 choices = df_colors$df$val,
                                                 choicesOpt = list(content = df_colors$df$img),
                                                 options=list(container="body"),
                                                 selected=df_colors$df$val[1]
                              ), NULL, "confusion matrix palette",options=list(container="body"))),
                       column(12,style="margin-top: 20px",
                              popify(bsButton("downp_cmrf", span(icon("fas fa-download"),icon("fas fa-image")),style  = "button_active"),NULL,"download CM plot",
                                     options=list(container="body")
  ...
  output$RFexp<-renderUI({
    div(style="background: white",
        tabsetPanel(
          ...
          tabPanel("2.3.4. Relationships",
                   uiOutput("rf_relations")
            ...
            div(style="background: white",
                tabsetPanel(
                  tabPanel(span("2.3.5.1. Variable Interactions"),
                           div(uiOutput("rf_inter"))),
                  tabPanel(span("2.3.5.2. Biplot Interactions"),
                           div(uiOutput("rf_interbiplot")))
                )
  ...
  output$rf_interbiplot<-renderUI({
    sidebarLayout(
      sidebarPanel(
        ...
        uiOutput("side_rf_grid")),
      mainPanel(
        withSpinner(type=8,color="SeaGreen",plotOutput("rfbiplot1"))
      )
    )
  })
  ...
  output$rf_interbiplot2<-renderUI({
    if(input$rf_useinter=="Interaction Frame"){
      div(
        span("+ Interaction:",
             div(
               pickerInput("rf_interaction", NULL, choices=c(rf_interactions_frame$df$interaction), width="200px")
             )
        )
      )
    } else{
      div(
        div(
          span("+ Variable 1",
               inline(
                 pickerInput("rf_grid_var1", NULL, choices=c(colnames(vals$RF_results$trainingData[-1])), width="100px")
               )
          )
        ),
        div(
          span("+ Variable 2",
               inline(
                 pickerInput("rf_grid_var2", NULL, choices=c(colnames(vals$RF_results$trainingData[-1])), width="100px", selected=colnames(vals$RF_results$trainingData[-1])[2])
               )
          )
        )
      )
  ...
  output$side_rf_grid<-renderUI({
    fluidRow(class="map_control_style",style="color: #05668D",
             ...
             div(
               span("+ Use",
                    inline(
                      pickerInput("rf_useinter", NULL, choices=c("Interaction Frame","Custom Variables"), width="150px", selected=colnames(vals$RF_results$trainingData[-1])[2])
                    )
               )
             ),
             uiOutput("rf_interbiplot2"),
             if(vals$RF_results$modelType=="Classification"){
               div(
                 span("+ Class",
                      inline(
                        pickerInput("rf_biplotclass", NULL, choices=levels(vals$RF_results$trainingData[,1]), width="150px")
             ...
             div(
               span(span(tipify(icon("fas fa-question-circle"),"Integer giving the number of equally spaced points to use for the continuous variables", options =list(container="body")),'+ Grid resolution'),
                    inline(
                      numericInput('rfbi_grid',NULL,20, width="75px")
                    )
               )
             ),
             div(
               actionLink('downp_rfbi',span("+ Download plot",icon("fas fa-image"),icon("fas fa-download")), style="button_active")
             )
  ...
  output$rfbiplot1<-renderPlot({
    ...
    if(input$rf_useinter=='Interaction Frame'){
      ...
      pic<-which(rf_interactions_frame$df$interaction==input$rf_interaction)
      var1<-rf_interactions_frame$df$variable[pic]
      var2<-as.character(rf_interactions_frame$df$root_variable[pic])
    }else{
  ...
  output$rf_relations<-renderUI({
    validate(need(input$go_rfplot  %% 2,"Use '+ Click to Measure Importance' in '2.3.1 Measures' panel before generating the plot  "))
    tabsetPanel(
      tabPanel(
        ...
        div(style="background: white",
            div(
              span(
                ...
                #inline(tipify(actionButton("rfcomp_meas_downp",span(icon("fas fa-image"),icon("fas fa-download"))),"Download plot"))
              )
            ),
            uiOutput("rf_rel")
        ...
        div(style="background: white",
            div(
              span(
                ...
                #inline(tipify(actionButton("rfcomp_rank_downp",span(icon("fas fa-image"),icon("fas fa-download"))),"Download plot"))
  ...
  output$rf_mindepth<-renderUI({
    ...
    sidebarLayout(
      sidebarPanel(
        ...
        uiOutput("side_rf_mindeph")),
      mainPanel(plotOutput("prf"))
    )
  })
  ...
  output$rf_measure<-renderUI({
    sidebarLayout(
      sidebarPanel(
        strong("Measure importance",tipify(icon("fas fa-question-circle"),"Creates a data frame with various measures of importance of variables in a random forest", options =list(container="body")), style="font-size: 16px"),
        uiOutput("rf_sidemeasure")
      ),
      mainPanel(
        fluidRow(
          tags$style('#rf_measure_out td {padding: 0}'),
          inline(
            DT::dataTableOutput("rf_measure_out")
          )
        )
      )
    )
  ...
  output$rf_measure_out<-DT::renderDataTable(data.frame(mindeaphrf()[[2]]),options = list(pageLength = 20, info = FALSE,lengthMenu = list(c(20, -1), c( "20","All")), autoWidth=T), rownames = TRUE,class ='cell-border compact stripe')
    ...
    fluidRow(
      ...
      div(
        span(span(tipify(icon("fas fa-question-circle"),"The sample of trees on which conditional mean minimal depth is calculated", options =list(container="body")),'+ mean_sample'),
             inline(
               pickerInput('rfmeasure_mean_sample',NULL,choices=c("all_trees", "top_trees", "relevant_trees"),selected='top_trees', width="75px")
             )
        )
      ),
      div(
        span(span(tipify(icon("fas fa-question-circle")," the measures of importance to be used", options =list(container="body")),'+ measures:'),
             p(
               fluidRow(
                 ...
                 inline(checkboxGroupInput('rf_measures',NULL,choices=c(choices_rfmeasure()),selected=choices_rfmeasure(),width="140px"))
      ...
      div(
        (actionLink('go_rfplot',span("+ Click to Measure Importance ",icon("fas fa-arrow-circle-right")), style = "animation: glowing3 1000ms infinite;"))
      ),
      conditionalPanel("input.go_rfplot % 2",{
        div(
          tipify(
            actionLink(
              'downcenter_rfdepth',span("+ Download Results",icon("fas fa-table")), style="button_active"
            ),
  ...
  output$rf_inter<-renderUI({
    ...
    sidebarLayout(
      sidebarPanel(
        ...
        uiOutput("rf_sideinter")),
      mainPanel(
        conditionalPanel("input.rf_intermode=='Plot'",{
          plotOutput("rf_inter_out")
        }),

        conditionalPanel("input.rf_intermode=='Table'",{
          fluidRow(
            tags$style('#rf_inter_tab td {padding: 0}'),
            inline(
              DT::dataTableOutput("rf_inter_tab")
            )
          )
        })
  ...
  output$rf_inter_tab<-DT::renderDataTable(data.frame(rf_interactions_frame$df),options = list(pageLength = 20, info = FALSE,lengthMenu = list(c(20, -1), c( "20","All")), autoWidth=T), rownames = F,class ='cell-border compact stripe')



  rf_inter<-reactiveValues(df=0)
  rf_interactions_frame<-reactiveValues(df=0)
  output$rf_inter_out<-renderPlot({
    ...
    withProgress(message = "Running ...",
                 ...
                 max = 1,
                 {
                   importance_frame<-mindeaphrf()[[2]]
                   vars <- important_variables(importance_frame, k = input$rfinter_k, measures = c("mean_min_depth", "no_of_trees"))
                   ...
                   rf_interactions_frame$df<-interactions_frame[ order(interactions_frame$occurrences,decreasing=T),]
                   ...
                   p<-plot_min_depth_interactions(interactions_frame,k=k2)
                   rf_inter$df<-p
                   p
                 })
    ...
    fluidRow(
      ...
      div(
        span(span(tipify(icon("fas fa-question-circle"),"The number of variables to extract", options =list(container="body")),'+ k'),
             inline(
               numericInput('rfinter_k',NULL,value=5,step=1, width="75px")
             )
        )
      ),
      div(
        span(span(tipify(icon("fas fa-question-circle"),"The sample of trees on which conditional mean minimal depth is calculated", options =list(container="body")),'+ mean_sample'),
             inline(
               pickerInput('rfinter_mean_sample',NULL,choices=c("all_trees", "top_trees", "relevant_trees"),selected='top_trees', width="80px")
             )
        )
      ),
      div(
        span(span(tipify(icon("fas fa-question-circle"),"The sample of trees on which unconditional mean minimal depth is calculated", options =list(container="body")),'+ unc_mean_sample'),
             inline(
               pickerInput('uncond_mean_sample',NULL,choices=c("all_trees", "top_trees", "relevant_trees"),selected='mean_sample', width="45px")
             )
        )
      ),
      div(
        span(span(tipify(icon("fas fa-question-circle"),"The number of best interactions to plot, if empty then all plotted", options =list(container="body")),'+ N inter'),
             inline(
               numericInput('rfinter_k2',NULL,value=30,step=1, width="75px")
             )
        )
      ),
      div(
        tipify(
          actionLink(
            'create_rfinter',span("+ Create Datalist",icon("fas fa-file-signature")), style="button_active"
          ),
      ...
      div(
        (actionLink('rfinter_downp',span("+ Download plot",icon("fas fa-image"),icon("fas fa-download")), style="button_active"))
      ),
      div(
        tipify(
          actionLink(
            'downcenter_rfinter',span("+ Download Interaction Results",icon("fas fa-table")), style="button_active"
          ),
  ...
  observeEvent(input$create_rfinter,{
    vals$hand_save<-"Create a datalist with the variables included in the N most frequent interactions in the RF"
    vals$hand_save2<-NULL
    vals$hand_save3<-NULL
    showModal(
      hand_save_modal()
  ...
  rf_rank<-reactiveValues(df=0)
  output$rf_rank<-renderUI({
    div(
      renderPlot({


        plot_importance_rankings(mindeaphrf()[[2]])
        #rf_rank$df<-recordPlot()
        #rf_rank$df
    ...
    div(
      renderPlot({
        withProgress(message = "Running ...",
                     ...
                     max = 1,
                     {
                       plot_importance_ggpairs(mindeaphrf()[[2]])
                       #rf_rel$df<-recordPlot()
                       #rf_rel$df
  ...
  output$side_rf_mindeph<-renderUI({
    ...
    fluidRow(class="map_control_style",style="color: #05668D",
             ...
             div(
               span("+",
                    inline(
                      checkboxInput("rf_depth", "Only significant variables:", T, width="75px")
                    )
               )
             ),
             inline(uiOutput("npicrf")),
             div(
               span(span(tipify(icon("fas fa-question-circle"),"The minimal number of trees in which a variable has to be used for splitting to be used for plotting", options =list(container="body") ),'+ Min_n_trees:'),
                    inline(
                      numericInput('depth_min_no_of_trees',NULL,value=0,step=1, width="75px")
                    )
               )
             ),
             div(
               span(span( tipify(icon("fas fa-question-circle"),"The sample of trees on which mean minimal depth is calculated", options =list(container="body")),'+ Mean_sample:'),
                    inline(
                      pickerInput('depth_mean_sample',NULL,choices=c( "top_trees","all_trees","relevant_trees"), selected= "top_trees",width='85px')
                    )
               )
             ),
             div(
               span(span(tipify(icon("fas fa-question-circle"),"Significance level", options =list(container="body")),'+ Sig:'),
                    inline(
                      numericInput("sigprf", NULL, 0.05, width="75px")
                    )
               )
             ),
             div(
               span("+ Plot size:",
                    inline(
                      numericInput("labrfsize",NULL, value=10, width="75px")
                    )
               )
             ),
             div(
               tipify(
                 actionLink(
                   'create_rf',span("+ Create Datalist",icon("fas fa-file-signature")), style="button_active"
                 ),
             ...
             div(
               actionLink(
                 'downp_prf',span("+ Download Plot",icon("fas fa-image"),icon("fas fa-download")), style="button_active"
               )
             )
  ...
  observeEvent(input$create_rf,{
    vals$hand_save<-"Create a datalist with the variables selected in the Random Forest Explainer"
    vals$hand_save2<-NULL
    vals$hand_save3<-NULL
    showModal(
      hand_save_modal()
  ...
  output$npicrf <- renderUI({
    ...
    span(span(tipify(icon("fas fa-question-circle", style="color: gray"),"The maximal number of variables with lowest mean minimal depth to be used for plotting", options =list(container="body")),'+ Number of variables:'),
         inline(
    ...
    column(12,
           column(12,style="margin-left: -20px; margin-bottom: 10px",
                  ...
                  column(11,style="margin-left: -15px; margin-bottom: -15px",
                         radioButtons("rf_type", NULL, c("Classification", "Regression"), inline = T, selected=vals$cur_rftype)
                  ),
           ...
           column(12,
                  if(input$rf_tab=="rf_tab1"){
                    fluidRow(
                      splitLayout(
                        selectInput("data_rfY","Y Datalist:",choices =  names(vals$saved_data), selected=vals$cur_rfY),
                        uiOutput('rf_supervisor'),
                        uiOutput('data_rfX_out')

                      ),
                      splitLayout(
                        uiOutput('rf_test_part'),
                        uiOutput("rf_partition")
                      )
                    )
                  } else{
                    splitLayout(
                      ...
                      splitLayout( cellWidths = c("70%","15%","15%"),
                                   uiOutput("rf_results_menu"),
                                   uiOutput("saverf_button"),
                                   uiOutput("rmrf_button")
                      )
  ...
  output$rf_supervisor <- renderUI({
    ...
    selectInput(
      "rf_sup",
      strong("Independent variable: (Y)", tiphelp('The response vector')),
      choices =choices ,
      selectize=T, selected=vals$cur_rfsup
    )

  })

  output$data_rfX_out<-renderUI({
    vals$bag_rf_data<-names(vals$saved_data)
    selectInput("data_rfX",strong("~ Training data (X):"),choices =vals$bag_rf_data,selectize=T, selected=vals$cur_data)
  })


  output$rf_test_part<-renderUI({
    req(input$data_rfX)
    selectInput("rf_test_partition",strong("Partition:",tiphelp("choose a factor as reference for the partition")), choices=c("None", colnames(attr(vals$saved_data[[input$data_rfX]],"factors"))),selected=vals$cur_partrf)
  })
  ...
  output$rf_partition<-renderUI({
    ...
    fac<-attr(vals$saved_data[[input$data_rfX]],"factors")[,input$rf_test_partition]
    choices<-levels(fac)

    selectInput("testdata_rf",strong("Test reference:",tiphelp("choose the level as reference for the test data. Data referring to the level of the chosen factor will not be considered in the training, and can be be later used to generate predictions")), choices=choices, selected=vals$cur_testrf)
  })
  ...
  output$rf_results_menu<-renderUI({
    column(12,
           selectInput("rf_models",strong("RF results:", tiphelp("Random Forest Results. Click to select RF results saved in the Training Datalist (X).")), choices=c("new rf (unsaved)", names(attr(vals$saved_data[[input$data_rfX]],"rf"))),selected=vals$cur_rf)
    )
  })

  output$saverf_button<-renderUI({
    req(!isFALSE(vals$bag_rf))
    div(style="margin-top: 20px",
        tipify(actionButton("tools_saverf", icon("fas fa-save")), "Save the rf model in the training Datalist (X)")
    )
  })
  ...
  output$rmrf_button<-renderUI({
    ...
    div(style="margin-top: 20px",
        tipify(actionButton("tools_rmrf", icon("far fa-trash-alt")), "Remove the rf model from the training Datalist (X)")
    )
  ...
  observeEvent(input$tools_saverf,{
    if(input$tools_saverf %% 2){

      vals$hand_save<-"Save new rf in"
  ...
  output$pick_partition_rf<-renderUI({
    if(input$rf_test_partition!="None"){
      em("Test data:",input$rf_test_partition,"::",input$testdata_rf)} else{

        em(input$rf_test_partition)
      }
  })
  ...
  output$rf_errors<-renderUI({
    ...
    column(12,
           ...
           sidebarLayout(
             sidebarPanel(uiOutput("RF_prederrors_side")),
             mainPanel(uiOutput("RF_prederrors"))
           )
  ...
  output$RF_prederrors_side<-renderUI({
    ...
    fluidRow( class="map_control_style",style="color: #05668D",
              div(
                tipify(
                  actionLink(
                    'down_rf_errors',span("+ Download",icon("fas fa-download")), style="button_active"
                  ),
              ...
              div(
                tipify(
                  actionLink(
                    'create_rfreg',span("+ Create Datalist",icon("fas fa-file-signature")), style="button_active"
                  ),
  ...
  observeEvent(input$down_rf_errors,{
    pic<-switch(vals$RF_results$modelType,
                "Regression"='RF regression - prediction errors',
                "Classification" ='RF classification - prediction errors'
  ...
  observeEvent(input$create_rfreg,{
    vals$hand_save<-"Create a datalist with the prediction errors"
    vals$hand_save2<-NULL
    vals$hand_save3<-NULL
    showModal(
      hand_save_modal()
    ...
    fluidRow(
      column(12, style = "background: Snow;",
             splitLayout(
               column(12,
                      strong("Palette"),
                      pickerInput(inputId = "cm_suprf_palette",
                                  label = NULL,
                                  choices =     df_colors$df$val[getgrad_col()],
                                  choicesOpt = list(content =     df_colors$df$img[getgrad_col()]), options=list(container="body"))
               ),
               fluidRow(style="margin-top: 20px",
                        popify(bsButton("downp_confrf", span(icon("fas fa-download"),icon("fas fa-image")),style  = "button_active"),NULL,"download CM plot",
                               options=list(container="body")
  ...
  output$conf_rf<-renderUI({
    ...
    column(12,
           ...
           renderPlot({
             res<-plotCM(conf/sum(conf)*100, input$cm_suprf_palette,  newcolhabs=newcolhabs$df)
             vals$conf_rf<-res
             res
           }),
  ...
  test_rf<-reactive({
    pred_tab<-if(input$rfpred_which=="Partition"){attr(vals$RF_results,"test")}
      ...
      sidebarPanel(
        fluidRow(class="map_control_style",style="color: #05668D",
                 div(
                   tipify(
                     actionLink(
                       'create_rfreg_pred',span("+ Create Datalist",icon("fas fa-file-signature")), style="button_active"
                     ),
                 ...
                 div(
                   tipify(
                     actionLink(
                       'down_rf_pred_results',span("+ Download",icon("fas fa-download"),icon("fas fa-table")), style="button_active"
                     ),
  ...
  observeEvent(input$create_rfreg_pred,{
    vals$hand_save<-"Create a datalist with the RF predictions"
    vals$hand_save2<-NULL
    vals$hand_save3<-NULL
    showModal(
      hand_save_modal()
  ...
  output$predictions_rf<-renderUI({
    ...
    column(12,style="background: white",
           splitLayout(
             ...
             uiOutput("rf_pred_type"),
             conditionalPanel("input.rfpred_which =='Datalist'",{
               div(
                 selectInput("predrf_new","Datalist",names(vals$saved_data[getnewdataRF()]))
               )
             })
           ),
  ...
  output$rf_pred_type<-renderUI({
    if(attr(vals$RF_results,'test')!="None"){
      radioButtons("rfpred_which","New data (X):",choices=c("Partition","Datalist"), inline=T)
    } else{ radioButtons("rfpred_which","New data (X):",choices=c("Datalist"),inline=T) }
  })
  ...
  output$rf_sidemulti<-renderUI({
    fluidRow(
      ...
      div(
        span("+",
             inline(checkboxInput("rf_sigmulti", "Only significant variables", T))
        )
      ),
      inline(
        div(
          span(
            inline(uiOutput("multi_no_labels"))
          )
        )
      ),
      div(
        span(span(tipify(icon("fas fa-question-circle"),"The measure of importance to be shown on the X axis", options =list(container="body")),'+ X:'),
             inline(pickerInput('multi_x_measure',NULL,choices=c("mean_min_depth","accuracy_decrease"), selected="mean_min_depth",width="140px"))
        )
      ),
      div(
        span(span(tipify(icon("fas fa-question-circle"),"The measure of importance to be shown on the Y axis", options =list(container="body")),'+ Y:'),
             inline(pickerInput('multi_y_measure',NULL,choices=c("times_a_root","gini_decrease"), selected="times_a_root",width="140px"))
        )
      ),
      div(
        span(span(tipify(icon("fas fa-question-circle"),"Exclude text labels that overlap too many things", options =list(container="body")),'+ max.overlaps:'),
             inline(numericInput('max.overlaps',NULL,10, width="40px"))
        )
      ),
      #div(span(inline(pickerInput('+ Multi_size_measure:',span('size_measure', tipify(icon("fas fa-question-circle"),"The measure of importance to be shown as size of points (optional)", options =list(container="body"))),choices=c("","p_value"), selected="times_a_root",width="100px")))),
      #div(span(inline(numericInput('+ Multi_min_no_of_trees ',strong('min_no_of_trees', tipify(icon("fas fa-question-circle"),"The minimal number of trees in which a variable has to be used for splitting to be used for plotting", options =list(container="body"))),value=0,step=1)))),
      div(
        span(span(tipify(icon("fas fa-question-circle"),"Significance level", options =list(container="body")),'+ Sig:'),
             inline(numericInput("sigmrf", NULL, 0.05, width="75px"))
        )
      ),

      div(
        (actionLink('downp_mrf',span("+ Download plot",icon("fas fa-image"),icon("fas fa-download")), style="button_active"))
      )
  ...
  output$rf_multi<-renderUI({
    ...
    sidebarLayout(
      sidebarPanel(
        ...
        uiOutput("rf_sidemulti")),
      mainPanel(plotOutput("rf_multi_out"))
    )
  })
  ...
  rf_multi<-reactive({
    ...
    p<-suppressWarnings(
      prf_multi(
        mindeaphrf(), sigs =if (input$rf_sigmulti == TRUE) {TRUE} else {input$multi_no_of_labels},
        input$sigmrf,
  ...
  output$multi_no_labels<- renderUI({
    ...
    div(
      span(span(tipify(icon("fas fa-question-circle"),"The approximate number of best variables (according to all measures plotted) to be labeled (more will be labeled in case of ties)", options =list(container="body")),'+ No_of_labels'),
           inline(
             numericInput('multi_no_of_labels',NULL,value=10,step=1, width="75px")
           )
      )
    )
  ...
  output$summ_out<-renderUI({
    column(12,switch (input$summ_options,
                      ...
                      "Datalist"=datalist_render(getdata_upload())
    )
    )
           ...
           span(
             inline(
               span(style="width: 150px",
                    ...
                    p(em(input$data_upload),strong("~", style="color: #0D47A1"))
               )
             ),
  ...
  observeEvent(input$revert_rda,{
    cur<-input$data_upload
    updatePickerInput(session,"data_upload",NULL,choices=names(vals$saved_data), selected=input$rda_X)
    updatePickerInput(session,"rda_X",strong("Y Datalist:", tiphelp("Response data")), choices=names(vals$saved_data)[-which(names(vals$saved_data)%in%input$data_upload)],selected=cur)
  ...
  output$rda_X<-renderUI({
    pickerInput("rda_X",span("X Data", tiphelp("Response data")), choices=names(vals$saved_data)[-which(names(vals$saved_data)%in%input$data_upload)])
  })
  rda_model<-reactive({
    data<-getdata_upload()
    x<-as.matrix(data)
    colnames(x)<-colnames(getdata_upload())
  ...
  output$rda_plot<-renderPlot({
    prda(rda_model(),
         ...
         col.sp=getcolhabs(newcolhabs$df,input$rda_spcolor,1),
    ...
    suppressWarnings(
      suppressMessages(
        plot(smw,w=dp_w, sig=input$dp_sig, z=input$dp_z,   BPs=dp_BPs,
             seq.sig=input$dp_seq.sig, bg= getcolhabs(newcolhabs$df,input$dp_palette,nlevels(as.factor(vals$splitBP[,1]))),bg_alpha=input$dp_bg,cols=c(getcolhabs(newcolhabs$df,input$dp_dcol,1),getcolhabs(newcolhabs$df,input$dp_scol,1),getcolhabs(newcolhabs$df,input$dp_bcol,1)))
      )
    )
  ...
  getDP<-reactive({
    ...
    data_empty<-data.frame(attr(vals$saved_data[[input$data_upload]],"factors")[,1,drop=F])
  ...
  output$save_bp<-renderUI({
    if(!isFALSE(vals$splitBP)){
      if(!any(unlist(lapply(attr(vals$saved_data[[input$data_upload]],"factors"), function (x) identical(x,as.vector(vals$splitBP)))))){
        popify(
          actionLink(
                           ...
                           span("+ Palette",
                                inline(
                                  pickerInput(inputId = "dp_palette",
                                              label = NULL,
                                              choices =     df_colors$df$val[getgrad_col()],
                                              choicesOpt = list(content =     df_colors$df$img[getgrad_col()]), options=list(container="body"), width="75px")
                                )
                           )
                         ),
                           ...
                           span("+ diss col",
                                inline(
                                  pickerInput(inputId = "dp_dcol",
                                              label = NULL,
                                              choices =   df_colors$df$val[getsolid_col()],
                                              choicesOpt = list(content =   df_colors$df$img[getsolid_col()]), options=list(container="body"), width="75px")
                                )
                           )
                         ),
                         div(
                           span("+ sig col",
                                inline(
                                  pickerInput(inputId = "dp_scol",
                                              label = NULL,
                                              choices =   df_colors$df$val[getsolid_col()],
                                              choicesOpt = list(content =   df_colors$df$img[getsolid_col()]), options=list(container="body"), selected=  df_colors$df$val[getsolid_col()][4], width="75px")
                                )
                           )
                         ),
                         div(
                           span("+ bp col",
                                inline(
                                  pickerInput(inputId = "dp_bcol",
                                              label = NULL,
                                              choices =   df_colors$df$val[getsolid_col()],
                                              choicesOpt = list(content =   df_colors$df$img[getsolid_col()]), options=list(container="body"), selected=  df_colors$df$val[getsolid_col()][3], width="75px")
                                )
                           )
                         ),
  ...
  output$fac_bp<-renderUI({
    fluidRow(
      renderPrint(
        which(!duplicated(sort(attr(vals$saved_data[[input$data_upload]],"factors")[,input$pw_sugg])))
      )
    )
  })
               ...
               conditionalPanel("input.segrda_view=='Plot'",{
                 div(
                            ...
                            div(
                              span("+ Variable Color:",
                                   inline(
                                     tipify(
                                       pickerInput(
                                         inputId = "segrda_spcolor",label = NULL,selected=  df_colors$df$val[getsolid_col()][4],choices =   df_colors$df$val[getsolid_col()],choicesOpt = list(content =   df_colors$df$img[getsolid_col()]),       options=list(container="body"), width="75px"), "Variable Color.")))
                            )
                     )
                   }),
                     ...
                     column(12,
                            ...
                            div(
                              span("+ Color:",
                                   inline(
                                     tipify(
                                       pickerInput(inputId = "segrda_colpalette",label = NULL,choices = df_colors$df$val,choicesOpt = list(content = df_colors$df$img),options=list(container="body"),selected=df_colors$df$val[1], width='75px'), "Symbol palette. Choose a gradient to color observations by a factor")))),
                   ...
                   conditionalPanel("input.segrda_show_labels % 2",{
                     column(12,
                            div(span("+ Factor:",
                                     inline(tipify(pickerInput("segrda_labfactor",NULL,choices = colnames(attr(getdata_upload(),"factors")), width="125px"),  "label classification factor")
                                     ))),
  ...
  output$fac_palette_pw<-renderUI({
    col<-getcolhabs(newcolhabs$df,input$segrda_colpalette,2)
    req(col[1]!=col[2])

    div(
      span("+ Factor:",
           inline(tipify(pickerInput("segrda_symbol_factor",NULL,choices = rev(colnames(attr(getdata_upload(),"factors"))), width='125px'), "symbol classification factor"))))
  })
  ...
  observeEvent(input$run_pwrda,{
    ...
    output$ppwRDA<-renderPlot({
      ...
      segrda<-prda(segrda_model()$rda.pw,
                   ...
                   col.sp=getcolhabs(newcolhabs$df,input$segrda_spcolor,1),
  ...
  output$segrda_X<-renderUI({
    pickerInput("segrda_X",span("X Data", tiphelp("Response data")), choices=names(vals$saved_data)[-which(names(vals$saved_data)%in%input$data_upload)])
  })
  ...
  observeEvent(input$revert_segrda,{
    cur<-input$data_upload
    updatePickerInput(session,"data_upload",NULL,choices=names(vals$saved_data), selected=input$segrda_X)
    updatePickerInput(session,"segrda_X",strong("Y Datalist:", tiphelp("Response data")), choices=names(vals$saved_data)[-which(names(vals$saved_data)%in%input$data_upload)],selected=cur)
  ...
  observeEvent(input$shp_add_base,{
    attr(vals$saved_data[[input$data_map]],"base_shape")<-filtershp()

  })
  observeEvent(input$shp_add_layer,{
    attr(vals$saved_data[[input$data_map]],"layer_shape")<-filtershp()
  })


  observeEvent(input$shp_extra_layer,{
    vals$hand_save<-"Add an Extra-Layer-Attribute to the Datalist"
    vals$hand_save2<-NULL
    vals$hand_save3<-NULL
    showModal(
      hand_save_modal()
  ...
  output$SHP_IN<-renderUI({
    req(length(vals$saved_data)>0)
    fluidRow(
      p(bsButton("shp_add_base",strong(popify(div("B",icon("fas fa-map")),NULL,paste0(span("Click to add shapefiles into the Datalist as a Base-Shape-Attribute"))),"Add", style='button_active'))),
      p(bsButton("shp_add_layer",strong(popify(div("L",icon("fas fa-map")),NULL,paste0(span("Click to add shapefiles into the Datalist as a Layer-Shape-Attribute"))),"Add", style='button_active'))
      ),
      p(bsButton("shp_extra_layer",strong(popify(div("E",icon("fas fa-map")),NULL,paste0(span("Click to add shapefiles into the Datalist as an Extra -Shape-Attribute"))),"Add", style='button_active'))
      )
    )
  })
  ...
  output$shp_feature1<-renderUI({
    ...
    splitLayout(
      selectInput('shp_feature1',"feature 1",choices=c("None",atributos_shp), selectize=T),
  ...
  output$shp_feature2<-renderUI({
    ...
    selectInput('shp_feature2',"feature 2",choices=c("None",lev_attrs), selectize=T)
  })
  ...
  output$pop_transform<-renderUI({
    column(12,
           h4(strong("Transformation tools"),style="border-bottom: 1px solid SeaGreen;"),
           column(12,
            ...
            conditionalPanel("input.mach_value % 2",{
              splitLayout(cellWidths = c("60%","40%"),
                          column(12,radioButtons(
                            "rares", NULL,
                            choiceValues  = c("Abund<","Freq<","Singletons"),
                            choiceNames  = list(
  ...
  output$cutlevel_obs<-renderUI({
    column(12,
           selectInput("cutlevel_obs", span("level",tipify(icon("fas fa-question-circle"),"The factor level to restric the observations")), levels(attr(vals$saved_data[[input$data_upload]],"factors")[, input$filter_data]), selectize=T)
    )
  })
  ...
  seldata <- reactive({
    ...
    if (length(input$selecvar) > 0) {
      data <- data[, input$selecvar, drop=F]
    }
    if (length(input$selecobs) > 0) {
      data <- data[input$selecobs, ]
    }
    data
  })
  ...
  l1<-reactive({
    list(
      ...
      col_factor= if(length(input$col_factor)>0){   getcolhabs(newcolhabs$df,input$col_factor,1) } else{ NULL},
      pt_legend=input$pt_legend,
      pt_legend= input$pt_legend,
      map_legend=input$map_legend,
      col_coords= if(length(input$col_coords)>0){   getcolhabs(newcolhabs$df,input$col_coords,1) } else{ NULL},
  ...
  map_data_disc <- reactive({
    ...
    m<-
      map_discrete_variable(
        ...
        col.coords=if(length(input$col_coords)>0){   getcolhabs(newcolhabs$df,input$col_coords,1) } else{ NULL},
        ...
        layer_col=getcolhabs(newcolhabs$df,input$layer_col,1),
        lighten=input$layer_lighten,
        base_col=getcolhabs(newcolhabs$df,input$base_col,1),
...
if(length(colors[which(trulayers)])>0){
  ...
  map_fac_disc<-reactive({
    ...
    m<-suppressWarnings(
      map_discrete_variable(
        ...
        cex.lab=input$pt_legend,
        leg=input$map_legend,
        col.coords=if(length(input$col_coords)>0){   getcolhabs(newcolhabs$df,input$col_coords,5)[1] } else{ NULL},
        col.palette=col,
        symbol=as.numeric(input$pt_symbol),
        scalesize_size= F,
        scalesize_color=F,
  ...
  map_data_interp<-reactive({
    ...
    p<-map_style(data=data,
                 ...
                 factors=labcoords(),
                 cex.fac=input$pt_factor+2,
                 col.fac=if(length(input$col_factor)>0){   getcolhabs(newcolhabs$df,input$col_factor,1) } else{ NULL},
                 showcoords=input$showcoords,
                 col.coords=if(length(input$col_coords)>0){   getcolhabs(newcolhabs$df,input$col_coords,1) } else{ NULL},
                 cex.coords=input$pt_coords+1,
                 showguides=input$showguides,
  ...
  output$persp_out<-renderUI({
    ...
    fluidRow(
      renderPlot({
        ...
        if(is.factor(data_z)){
          colors<- getcolhabs(newcolhabs$df,input$pt_palette,nlevels(data_z))
        } else {
          colors<- getcolhabs(newcolhabs$df,input$pt_palette,length(my_rst@data@values))
        }
        ...
        if(isTRUE(input$showcoords)){
          ...
          points(mypoints,pch = 3,col = if(length(input$col_coords)>0){   getcolhabs(newcolhabs$df,input$col_coords,1) } else{ NULL}, cex=input$pt_coords)
        }
        if(isTRUE(input$showfactors)){
          ...
          mypoints <- trans3d(xx,yy,zz,pmat = pmat)
          text(mypoints,labels=labels,pch = 3,col = if(length(input$col_factor)>0){   getcolhabs(newcolhabs$df,input$col_factor,1) } else{ NULL}, cex=input$pt_factor)
        }
        if(isTRUE(input$showguides)){
          grid4D(coords,my_rst, col= getcolhabs(newcolhabs$df,input$col_guides,1), pmat=pmat,lty=as.numeric(input$lty_guides),lwd=input$lwd_guides)
        }
        persp<-recordPlot()
  ...
  get_stackmap<-reactive({
    ...
    pmat<-stack4D_2(res_list,
                    ...
                    coords=coords,
                    labels=labs,
                    col.labels=if(length(input$col_factor_stack)>0){   getcolhabs(newcolhabs$df,input$col_factor_stack,1) } else{ NULL},
                    cex.labels=input$pt_factor_stack,
                    col.coords=if(length(input$col_coords_stack)>0){   getcolhabs(newcolhabs$df,input$col_coords_stack,1) } else{ NULL},
                    cex.coords=input$pt_coords_stack,
                    show_coords=co,
                    show_labels=showlab,
                    newcolhabs=newcolhabs$df,
  ...
  output$m3d_map<-renderRglwidget({
    ...
    rgl.open()
    rgl.bg(color=c('white','white'))
    anim_4D(r1=p1,r2=p2,r3=NULL,colors=input$pt_palette, exp=input$surf_exp,   newcolhabs=newcolhabs$df)
    rglwidget()
  ...
  observeEvent(input$trainRF,{
    t<-try({
      ...
      envi_o<-envi
      factors<-attr(envi,"factors")

      if(input$rf_test_partition!="None"){
        fac<-factors[,input$rf_test_partition,drop=F]
        pic<-which(fac==input$testdata_rf)
        factors<-factors[pic,]
        pic0<-rownames(envi)%in%rownames(factors)
        envi=envi[which(!pic0),]
        sup=sup[rownames(envi),, drop=F]
        test<-which(pic0)
  ...
  observe({
    ...
    if(input$rf_models=="new rf (unsaved)"){
      vals$RF_results<-vals$rf_unsaved } else{
        ...
        vals$RF_results<-attr(vals$saved_data[[input$data_rfX]],"rf")[[input$rf_models]]
      }
    rf_interactions_frame$df<-0
  })
  ...
  prf.reactive <- reactive({
    ...
    res<-prf(mindeaphrf(), sigs =     if (input$rf_depth == TRUE) { TRUE} else {input$n_var_rf},
             input$sigprf,
          ...
          sidebarPanel(
            fluidRow( class="map_control_style",style="color: #05668D",
                      div(
                        tipify(
                          actionLink(
                            'downcenter_intererror',span("+ Download",icon("fas fa-download")), style="button_active"
                          ),
                          "Download error results",options=list(container="body")
                        )
                      ...
                      div(
                        tipify(
                          actionLink(
                            'create_rfreg_errorinter',span("+ Create Datalist",icon("fas fa-file-signature")), style="button_active"
                          ),
                          "Create a datalist with the model errors", options=list(container="body")
                        )
                      )
            )
          ),
          mainPanel(
            uiOutput("rfreg_intererrors"),
            uiOutput("rfclass_intererrors"),
          )
        )
      )
    )
  ...
  observeEvent(input$create_rfreg_errorinter,{
    vals$hand_save<-"Create a datalist with the model errors"
    vals$hand_save2<-NULL
    vals$hand_save3<-NULL
    showModal(
      hand_save_modal()
  ...
  observeEvent(input$downcenter_intererror,{
    pic<-switch(vals$RF_results$modelType,
                "Regression"='RF model regression errors',
                "Classification" ='RF model classification errors'
  ...
  rfmodel_errors<-reactiveValues(df=0)

  output$rfreg_intererrors<-renderUI({
    req(vals$RF_results$modelType=="Regression")
    renderPrint({accu_rf_reg_model(vals$RF_results)})
  })
  output$rfclass_intererrors<-renderUI({
    req(vals$RF_results$modelType=="Classification")
    renderPrint({accu_rf_class(vals$RF_results)})
  })
  ...
  output$rfsummary <- renderUI({
    req(length(vals$RF_results)>0)
    div(
      p(popify(downloadButton("down_rf_results", span(icon("fas fa-tree"),icon("fas fa-tree", style = "margin-left: -10px;"),icon("fas fa-tree", style = "margin-left: -10px;")),style = "button_active"),NULL,"download rf results as rds file",
               options=list(container="body"))),
      p(strong('Training data (X):'),em(attr(vals$RF_results,"Datalist"))),
      p(strong('Independent variable (Y):'),em(attr(vals$RF_results,"Y"))),
      p(strong('Partition:'),em(attr(vals$RF_results,"test_partition"))),
      renderPrint(gettrain(vals$RF_results))
             ...
             div(span("+ Border", inline(
               tipify(
                 pickerInput(inputId = "bmu_border_grid",
                             ...
                             choicesOpt = list(
                               content =  df_colors$df$img[getsolid_col()] ),
                             selected= df_colors$df$val[getsolid_col()] [7], width="75px"),
                 "Grid border color"
               )
             ))),
             div(span("+ Var color:",inline(
               tipify(
                 pickerInput(
                   ...
                   choices =  df_colors$df$val[getsolid_col()] ,
                   choicesOpt = list(
                     content =  df_colors$df$img[getsolid_col()] ), width="75px",
                   options=list(container="body")
                 ),
                 "Variable color"
               )
  ...
  output$bmu_legend_out<-renderUI({
    ...
    col<-getcolhabs(newcolhabs$df,input$bmu_facpalette,2)
    req(col[1]!=col[2])
  ...
  output$pclus_legcontrol<-renderUI({
    col<-getcolhabs(newcolhabs$df,input$pclus_facpalette,2)
    req(col[1]!=col[2])
    ...
    {span("+ Labels",
          inline(
            tipify(pickerInput("bmu_factors",NULL,
                               choices = c(colnames(attr(getdata_som(),"factors"))), selected=vals$bmu_factors, width="75px"),"color observations by factor")
          )
    )
    }})
  ...
  BMUs<-reactive({
    ...
    p <-pcorr(
      ...
      border=getcolhabs(newcolhabs$df,input$bmu_border_grid,1),

      col.text= getcolhabs(newcolhabs$df,input$bmu_var_color,1),
      newcolhabs=newcolhabs$df,
  ...
  output$Locate_NA<-renderUI({
    req(anyNA(unlist(getdata_upload())))
    div(
  ...
  output$summ_fac<-renderPlot({
    data=getdata_upload()
    factors<-data[,unlist(lapply(data,is.factor))]
    str_factors(factors,newcolhabs=newcolhabs$df)
  ...
  output$strlabels <- renderPrint({
    ...
    ppsummary("----------------")
    ppsummary(paste("Missing values:",sum(is.na(attr(getdata_upload(),"factors")))))
    ppsummary("----------------")
            ...
            splitLayout(
              ...
              column(12,
                     ...
                     pickerInput(inputId = "hcdata_palette",
                                 label = NULL,
                                 choices = df_hc$df$val,
                                 choicesOpt = list(content = df_hc$df$img)))
  ...
  output$dt_panel<-renderUI({
    ...
    column(12,
           column(12,style="margin-top: 20px",
                  splitLayout(
                    radioButtons("dt_type","Type",c("Classification","Regression", selected=vals$dt_type)),
                    uiOutput("data_dtY"),
                    uiOutput("sup_dt"),
                    uiOutput("data_dtX")
                  )),
           tabsetPanel(id="dt_tab",
                       tabPanel(strong("1. Training"),value="dt_tab1",
                                column(12,style="background: white; margin: 20 20 20 20",
                                       column(12,uiOutput("controldt")))),
                       tabPanel(strong("2. Results"),value="dt_tab2",
                                tabsetPanel(
                                  tabPanel(strong('2.1. Summary'),
                                           verbatimTextOutput('dt_summary')),
                                  tabPanel(strong("2.2. Confusion Matrix"), uiOutput("confusion_DT")),
                                  tabPanel(strong("2.3 Plot"),
                                           column(12,
                                                  column(12,actionButton('downp_dt',icon("fas fa-image"),icon("fas fa-download"), style="button_active")),
                                                  column(12,pickerInput(inputId = "dt_palette",
                                                                        label = NULL,
                                                                        choices = df_colors$df$val,
  ...
  output$dt_summary <- renderPrint({
    gettrain(vals$DT_results)
  })

  output$confusion_DT<-renderUI({
    column(12,
           renderPlot({
             plotCM(vals$DT_results, input$dt_palette,  newcolhabs=newcolhabs$df)
           }))
  })
  getdata_dtY<-reactive({
    data<-vals$saved_data[[input$data_dtY]]
    data
  })
  getdata_dtX<-reactive({
    data<-vals$saved_data[[input$data_dtX]]
    data
  })

  output$data_dtY<-renderUI({
    selectInput("data_dtY",strong("Y Datalist:"),choices=names(vals$saved_data), selectize=T)
  })
  output$data_dtX<-renderUI({
    selectInput("data_dtX",strong("~ Training data (X)"),choices=names(vals$saved_data), selectize=T, selected=vals$cur_data)
  })
  output$sup_dt<-renderUI({
    if(input$dt_type=="Classification")
    {
      selectInput("sup_fac_dt",tipify(strong("Independent variable (Y):"),"The response vector"),choices=colnames(attr(getdata_dtY(),"factors")), selectize=T)
    } else {
      selectInput("sup_data_dt",tipify(strong("Independent variable (Y):"),"The response vector"),choices=colnames(getdata_dtY()), selectize=T)
  ...
  observeEvent(input$trainDT,{
    rfs = get_supdt()
    withProgress(message = "Running DT... the time taken will depend on the selected tuning",
                 min = 1,
                 max = 1,
                 {
                   vals$DT_results<- wrapDT(
                     rfs,
                     seed = input$seed_dt,
                     trainControl.args = list(
                       method = input$res_method_dt,
                       number = input$cv_dt,
                       repeats = input$repeats_dt,
                       p=input$pleave_dt/100
                     )
                   )
                 })

    updateTabsetPanel(session,'dt_tab','dt_tab2')
  ...
  decision_tree <- reactive({
    ptree_caret(vals$DT_results,palette=input$dt_palette, newcolhabs=newcolhabs$df)
    vals$ptree_plot<-recordPlot()
  })
  ...
  output$controldt <- renderUI({
    fluidRow(
      column(
        12,
        splitLayout(

          numericInput("seed_dt", strong("seed",tipify(a(icon("fas fa-question-circle")), textseed(), options=list(container="body"))), value = NULL)
        ),
        splitLayout(cellWidths = c("30%","30%"),
                    column(12,
                           radioButtons("res_method_dt","Resampling method", choiceValues=list("boot","cv","repeatedcv","LOOCV","LGOCV"), choiceNames=list(
                    ...
                    column(12,
                           splitLayout(
                             conditionalPanel("input.res_method_dt=='cv'|input.res_method=='repeatedcv'",{
                               numericInput("cv_dt", strong("cv", tipify(a(icon("fas fa-question-circle")),"number of folds", options=list(container="body"))), value = 5)
                             }),
                             conditionalPanel("input.res_method_dt=='repeatedcv'",{
                               numericInput("repeats_dt", strong("repeats", tipify(a(icon("fas fa-question-circle")),"the number of complete sets of folds to compute", options=list(container="body"))), value = 1)
                             })
                           ),
                           conditionalPanel("input.res_method_dt=='boot'",{
                             numericInput("cv_dt", strong("number", tipify(a(icon("fas fa-question-circle")),"the number of resampling iterations", options=list(container="body"))), value = 99)
                           }),
                           conditionalPanel("input.res_method_dt=='LGOCV'",{
                             numericInput("pleave_dt", strong("p (%)", tipify(a(icon("fas fa-question-circle")),"the training percentage", options=list(container="body"))), value = 10)})
                    )
        )
      ),
      column(12, align = "center", popify(actionButton("trainDT", h4(icon("fas fa-tree"),"run ctree",icon("fas fa-arrow-circle-right")), style = "background:  #05668D; color: white"),NULL,"Click to run")),
               ...
               span("+ Palette:",
                    inline(
                      pickerInput(inputId = "cm_supsom_palette",
                                  label = NULL,
                                  choices =     df_colors$df$val[getgrad_col()],
                                  choicesOpt = list(content =     df_colors$df$img[getgrad_col()]), options=list(container="body"),width='100px')
                    )
               )
             ),
             div(
  ...
  output$predsom_confusion<-renderPlot({
    conf<-get_predsom_cm()
    res<-plotCM(conf/sum(conf)*100, input$cm_supsom_palette,  newcolhabs=newcolhabs$df)
    vals$conf_som<-res
    res
  })
    ...
    fluidRow(
      ...
      div(span("+ Training color:",inline(
        pickerInput(
          ...
          label =NULL,
          choices =  df_colors$df$val[getsolid_col()] ,
          choicesOpt = list(content =  df_colors$df$img[getsolid_col()] ),
        ...
        fluidRow(
          ...
          tipify(
            pickerInput(
              ...
              choices =  df_colors$df$val[getsolid_col()] ,
              choicesOpt = list(content =  df_colors$df$img[getsolid_col()] ),
              selected= df_colors$df$val[getsolid_col()] [2],
              options=list(container="body")),
  ...
  BMUs_pred<-reactive({
    p<-pcorr(
      ...
      border=getcolhabs(newcolhabs$df,input$bmu_p_border_grid,1),
  ...
  output$som_partition<-renderUI({
    ...
    if(input$som_tab=="som_tab1"){
      div(
        pickerInput("som_test_pick",strong('Partition:', tiphelp("choose a factor from Datalist X as reference for data partioning")), choices=c("None", colnames(attr(vals$saved_data[[input$data_som]],"factors"))),selected=vals$cur_partsom,width="150px",)
      )
    }
  })
  ...
  cutsom.reactive<-reactive({
    ...
    m<-getmodel_hc()
    somC<-cutsom(m, picK(), method.hc = input$method.hc0, palette=input$hcmodel_palette,newcolhabs=newcolhabs$df)
    somC
  })
  bmu_factors_reac<-reactive({
    factors<-attr(getdata_som(),"factors")
    if (length(input$bmu_factors)>0) {
      factors[rownames(getdata_som()), vals$bmu_factors]
    } else{factors[,1]}
  })
